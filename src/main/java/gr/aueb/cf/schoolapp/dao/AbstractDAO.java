...
public class AbstractDAO <T extends IdentifiableEntity> implements IGenericDAO<T> {
    ...
    @Override
    public long count() {
        //το κανουμε με jpql , μπορει να γινει και με CriteriaAPI
        return getEntityManager()
                .createQuery("SELECT COUNT(e) FROM " + persistenceClass.getSimpleName() + " e",
                        Long.class)
                .getSingleResult();
    }

    @Override
    //στο Map criteria, το String ειναι το πεδίο και το Object ειναι το value
    public long getCountByCriteria(Map<String, Object> criteria) {
        CriteriaBuilder builder = getEntityManager().getCriteriaBuilder();
        //δινουμε σαν ορισμα στο createQuery τον τυπο δεδομενων που επιστρεφει
        CriteriaQuery<Long> countQuery = builder.createQuery(Long.class);
        Root<T> entityRoot = countQuery.from(persistenceClass);

        List<Predicate> predicates = getPredicatesList(builder, entityRoot,
                criteria);
        //η toArray ειναι typed, οποτε πρεπει να βαλουμε τι θελουμε να
        // επιστρεφει. εδω Predicate[] , το [0] δεν το παιρνει υπ οψιν
    ...
    public Optional<T> getById(Object id) {
        EntityManager em = getEntityManager();
        //χρησιμοποιουμε το ofNullable γιατι δεν ειμαστε σιγουροι αν υπαρχει
